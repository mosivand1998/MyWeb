...
class UserManager(BaseUserManager):
    ...
    def _create_user(self,username,phone_number,email,password,is_staff,is_superuser,**extra_fields):
        """
        creates ans saves a User with the given username, email and password.
        """
        now = timezone.now()
        if not username:
...
class User(AbstractBaseUser,PermissionsMixin):
                                ...
                                validators = [
                                    validators.RegexValidator(r'^[a-zA-Z][a-zA-Z0-9_\.]+$',
                                                              _('Enter a valid username starting with a-z.'
                                                                ...
                                                                'and undercore characters .'),'invalid'),
    ...
    phone_number = models.BigIntegerField(_('mobile number'),unique=True,null=True,blank=True,
                                          validators = [
                                              ...
                                              validators.RegexValidator(r'^989[0-3,9]\d{8}$',
                                                                        ('Enter a valid moile number.'),'invalid')
    ...
    def get_full_name(self):
        ...
        full_name = '%5 %5' % (self.first_name,self.last_name)
        return full_name.strip()

    def get_short_name(self):
        """
        return the short_name for the user.
    ...
    def is_loggedin_user(self):
        """
        returns true if user has actually logged in with valid credentiols.
        """
...
class UserProfile(models.Model):
    ...
    province =models.ForeignKey(verbose_name=_('province'),to='province',null=True, on_delete=models.SET)
